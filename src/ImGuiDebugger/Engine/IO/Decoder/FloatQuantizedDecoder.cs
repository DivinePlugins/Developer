...
internal sealed class FloatQuantizedDecoder : IDecoder<float>
{
    private const int QFE_ROUNDDOWN = 0x1;
    private const int QFE_ROUNDUP = 0x2;
    private const int QFE_ENCODE_ZERO_EXACTLY = 0x4;
    private const int QFE_ENCODE_INTEGERS_EXACTLY = 0x8;
    ...
    public FloatQuantizedDecoder(string fieldName, int bitCount, int flags, float minValue, float maxValue)
    {
        ...
        {
            LogManager.Debug($"flags changed for Field {fieldName}, [{this.flags}->{this.encodeFlags}]");
        }
    }
    ...
    private int computeEncodeFlags(int f)
    {
        ...
        if (!(minValue < 0.0f && maxValue > 0.0f))
        {
            ...
            {
                LogManager.Warn($"Field {fieldName} was flagged to encode zero exactly, but min/max range doesn't span zero [{minValue}->{maxValue}]");
            }
            f &= ~QFE_ENCODE_ZERO_EXACTLY;
        }
    ...
    private void initialize()
    {
        ...
        {
            LogManager.Warn($"Field {fieldName} was flagged to both round up and down, these flags are mutually exclusive [{minValue}->{maxValue}]\n");
        }
        ...
        if ((flags & QFE_ENCODE_INTEGERS_EXACTLY) != 0)
        {
            ...
            int trueRange = (1 << Util.calcBitsNeededFor(int.Max(delta, 1)));
            ...
            if (nBits > bitCount)
            {
                LogManager.Warn($"Field {fieldName} was flagged QFE_ENCODE_INTEGERS_EXACTLY, but didn't specify enough bits, upping bitcount from {bitCount} to {nBits} for range [{minValue}->{maxValue}]");
                bitCount = nBits;
                quanta = (1 << bitCount);
            }
        ...
        {
            throw new Exception("Assert failed: highLowMultiplier is zero!");
        }
    ...
    private float assignRangeMultiplier(int nBits, float range)
    {
        ...
        if ((long)(highLowMul * range) > highValue || (highLowMul * range) > (double)highValue)
        {
            ...
            {
                throw new Exception("Doh! We seem to be unable to represent this range.");
            }
        }
    ...
    private float quantize(float value)
    {
        if (value < minValue)
        {
            ...
            {
                LogManager.Warn($"Field {fieldName} tried to quantize an out-of-range value ({value}, range is {minValue}->{maxValue}), clamping.");
            }
