...
[Priority(86)]
internal sealed class Other(IMainMenu mainMenu, ILog log) : IDebuggerTool
{
    private readonly HashSet<OrderType> ignoreOrders =
    [
        OrderType.Cast,
    ...
    private readonly MenuSwitcher enabled = mainMenu.OnExecuteOrderMenu.AddSwitcher("Other", false).SetTooltip("Player.OnExecuteOrder");
    ...
    {
        if (this.ignoreOrders.Contains(e.Order.Type))
        {
            return false;
        }
